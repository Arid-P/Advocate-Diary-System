...
> [!IMPORTANT]
> **Source Code vs. Test Suite Attribution**:
> - **Backend Source Code**: Purely and entirely written by the author (**[Arid-P](https://github.com/Arid-P)**). All architectural planning, database configurations, SQLAlchemy 2.0 ORM models, Pydantic v2 validation schemas, CRUD data-access functions, FastAPI route handlers, and application integration (`main.py`) were handcrafted directly by the author.
> - **Automated Test Suite**: The automated test suite (`tests/` directory) was completely generated by an AI assistant to verify regression stability, database isolation, and API endpoint contracts.
...
| :--- | :--- | :--- |
| **Language** | Python 3.14+ | Modern type hints and annotations |
...
| **Testing** | Pytest | Comprehensive automated test suite with isolated test DB |
...
├── tests/                        # Automated Pytest suite (AI-generated)
...
### 1. Prerequisites
- [uv](https://github.com/astral-sh/uv) (recommended) and Python 3.14+
...
```bash
uv run uvicorn advocatediarysystem.main:app --reload
```
...
### 5. Running the Tests
Execute the full test suite (43 tests covering models, schemas, CRUD, and router endpoints):
```bash
uv run pytest -v -s
```
