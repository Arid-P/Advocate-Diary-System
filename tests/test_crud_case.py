...
def test_client(db_session):
    return crud_client.create_client(db=db_session, client_in=ClientCreate(name="Case Client"))

def test_create_case(db_session, test_client):
    case_in = CaseCreate(title="Test Case", description="Desc", client_id=test_client.id)
...
def test_get_case_by_id(db_session, test_client):
    case_in = CaseCreate(title="Get Case", client_id=test_client.id)
...
def test_get_cases_by_client(db_session, test_client):
    crud_case.create_case(db=db_session, case_in=CaseCreate(title="Case 1", client_id=test_client.id))
    crud_case.create_case(db=db_session, case_in=CaseCreate(title="Case 2", client_id=test_client.id))
    
    other_client = crud_client.create_client(db=db_session, client_in=ClientCreate(name="Other"))
    crud_case.create_case(db=db_session, case_in=CaseCreate(title="Other Case", client_id=other_client.id))
...
def test_update_case(db_session, test_client):
    created_case = crud_case.create_case(db=db_session, case_in=CaseCreate(title="Old Title", client_id=test_client.id))
...
def test_delete_case(db_session, test_client):
    created_case = crud_case.create_case(db=db_session, case_in=CaseCreate(title="To Delete", client_id=test_client.id))
